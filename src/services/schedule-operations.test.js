...
const schedules = [
  ["general-auto-0810", "GENERAL_AUTO", "08:10"], ["general-auto-1130", "GENERAL_AUTO", "11:30"], ["general-auto-1430", "GENERAL_AUTO", "14:30"], ["general-auto-1840", "GENERAL_AUTO", "18:40"],
].map(([id, type, time]) => ({ id, type, enabled: true, cadence: { kind: "daily", time }, nextRunAt: null }));
assert.deepEqual(getProductionScheduleReadiness(schedules), { ready: true, expectedCount: 4, activeCount: 4, unavailable: [] });
...
assert.deepEqual(normalizeScheduleHistory(history).map((run) => [run.type, run.result]), [["GENERAL_AUTO", "게시 완료"]]);
...
assert.deepEqual(failureHistory[1].failure, { stage: "UNKNOWN", code: "unknown_schedule_failure", message: "자동 실행 처리 중 문제가 발생했습니다." });
