...
function content() {
  return `<style>
    .app-schedule-layout{display:grid;gap:24px}.app-schedule-panel,.app-schedule-card{padding:22px;border:1px solid #e2e6ec;border-radius:14px;background:#fff}.app-schedule-panel h2,.app-schedule-card h2{margin:0 0 10px}.app-schedule-notice{background:#fff8e6;border-color:#f6d98b;color:#664d03}.app-schedule-form,.app-schedule-editor{display:grid;gap:12px;max-width:520px}.app-schedule-form label,.app-schedule-editor label{display:grid;gap:6px;font-weight:700}.app-schedule-form input,.app-schedule-form select,.app-schedule-editor input{font:inherit;padding:9px;border:1px solid #cfd6e2;border-radius:8px}.app-schedule-list{display:grid;gap:12px}.app-schedule-meta,.app-runtime-status{color:#667085;line-height:1.6}.app-schedule-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:14px}.app-schedule-button{border:1px solid #cfd6e2;border-radius:8px;background:#fff;color:#344054;cursor:pointer;font:inherit;font-weight:700;padding:9px 13px}.app-schedule-form .primary{border-color:#294d9a;background:#294d9a;color:#fff}</style>
  <div class="app-schedule-layout">
    ...
    <section class="app-schedule-panel"><h2>일정 추가</h2><form id="schedule-form" class="app-schedule-form"><label>이름<input name="name" maxlength="120" required></label><input name="type" type="hidden" value="GENERAL_AUTO"><label>실행 시간<input name="time" type="time" required></label><label><input name="enabled" type="checkbox"> 활성</label><button class="app-schedule-button primary" type="submit">추가</button></form></section>
