...
async function auth(request, env) { const result = await requireAdminApiSession(request, env); return result.ok ? null : result.response; }
...
export async function handleOperatorProducts(request, env, url, { list = getProducts, save = saveProduct } = {}) {
  const unauthorized = await auth(request, env); if (unauthorized) return unauthorized;
  try {
    if (request.method === "GET") {
      const active = queryBoolean(url.searchParams.get("active")); const eligible = queryBoolean(url.searchParams.get("eligible"));
      const products = (await list(env)).map(toOperatorProduct).filter((item) => (active === null || item.active === active) && (eligible === null || item.autoPostEligible === eligible));
      return ok({ products });
    }
    if (request.method !== "POST") return fail("Method Not Allowed", 405);
    const input = parseInput(await request.json());
    return ok({ product: toOperatorProduct(await save(env, toLegacy(input))) });
...
export async function handleOperatorProductById(request, env, productId, { get = getProductById, save = saveProduct } = {}) {
  ...
  try {
    const existing = await get(env, productId); if (!existing) return fail("Product not found", 404, { code: "product_not_found" });
    const input = parseInput(await request.json(), { partial: true });
    return ok({ product: toOperatorProduct(await save(env, toLegacy(input, existing))) });
