...
assert.match(legacyText, /쿠팡 상품 찾기/u);
assert.match(legacyText, /product-candidates/u);
assert.match(legacyText, /generate-content/u);
assert.match(legacyText, /Commerce 글 생성/u);
assert.match(legacyText, /renderCommerceDraft/u);
assert.match(legacyText, /storyDescriptor/u);
assert.match(legacyText, /storyMetadata/u);
assert.match(legacyText, /contentAngle/u);
assert.match(legacyText, /hookType/u);
assert.match(legacyText, /textarea\.value=draft\.text/u);
assert.match(legacyText, /Threads에 게시/u);
assert.match(legacyText, /publish-content/u);
assert.match(legacyText, /publishCommerceDraft/u);
assert.match(legacyText, /\/performance/u);
assert.match(legacyText, /renderPerformance/u);
assert.match(legacyText, /게시 성과를 불러올 수 없습니다/u);
assert.match(legacyText, /void loadPerformance/u);
assert.match(legacyText, /if\(!textarea\.value\.trim\(\)\)/u);
assert.match(legacyText, /button\.disabled=true/u);
assert.match(legacyText, /이 상품 선택/u);
assert.match(legacyText, /selectCoupangCandidate/u);
assert.match(legacyText, /직접 입력할 수 있습니다/u);
assert.match(legacyText, /formValue\('productName',candidate\.productName\)/u);
assert.match(legacyText, /formValue\('sourceUrl',candidate\.productUrl\)/u);
assert.match(legacyText, /formValue\('affiliateLink',candidate\.affiliateUrl\)/u);
