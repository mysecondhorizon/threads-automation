...
const response = await handleOperatorActivity(request("/api/activity?limit=12"), env, undefined, { getActivity: async (_env, options) => { receivedLimit = options.limit; return { items:[], limit:12, hasMore:false, generatedAt:"2026-08-29T00:00:00.000Z", partial:true }; } });
assert.equal(response.status, 200);
assert.equal(receivedLimit, "12");
assert.deepEqual(await response.json(), { ok:true, items:[], limit:12, hasMore:false, generatedAt:"2026-08-29T00:00:00.000Z", partial:true });
