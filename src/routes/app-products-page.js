...
function renderProductsPageContent() {
  ...
  <section class="opportunities-layout" aria-label="Product Opportunities">
    <div class="opportunities-toolbar">
      ...
      <button id="opportunity-discover" class="opportunity-button" type="button">AI 기회 탐색</button>
    ...
    <p class="opportunity-meta">최신 검색·소비 신호를 조사해 새로운 제품 기회 후보를 찾습니다.</p>
...
export function buildProductOpportunitiesClientScript() {
  return `(() => {
    const list=document.querySelector('#opportunity-list'),feedback=document.querySelector('#opportunity-feedback'),count=document.querySelector('#opportunity-count'),filter=document.querySelector('#opportunity-status-filter'),create=document.querySelector('#opportunity-create'),discover=document.querySelector('#opportunity-discover'),form=document.querySelector('#opportunity-form'),formTitle=document.querySelector('#opportunity-form-title'),formFeedback=document.querySelector('#opportunity-form-feedback'),cancel=document.querySelector('#opportunity-cancel');
    ...
    create.addEventListener('click',()=>openForm(null));cancel.addEventListener('click',()=>{form.hidden=true;editingId=null;formFeedback.textContent=''});filter.addEventListener('change',render);discover.addEventListener('click',async()=>{discover.disabled=true;feedback.dataset.state='';feedback.textContent='AI 기회 탐색 중…';try{const result=(await api('/api/product-opportunities/discover',{method:'POST'})).discovery;await load();feedback.textContent='새로 저장 '+result.savedCount+'개 · 추천 '+result.recommendedCount+'개 · 중복 건너뜀 '+result.skippedDuplicateCount+'개 · 유효하지 않음 '+result.skippedInvalidCount+'개';feedback.dataset.state='success'}catch(error){feedback.textContent=error.message;feedback.dataset.state='error'}finally{discover.disabled=false}});form.addEventListener('submit',async(event)=>{event.preventDefault();const save=form.querySelector('button[type="submit"]');save.disabled=true;formFeedback.dataset.state='';formFeedback.textContent='저장 중…';try{const data=await api(editingId?'/api/product-opportunities/'+encodeURIComponent(editingId):'/api/product-opportunities',request(editingId?'PATCH':'POST',payload()));form.hidden=true;editingId=null;formFeedback.textContent='';await load();if(data.opportunity)feedback.textContent='저장했습니다.';feedback.dataset.state='success'}catch(error){formFeedback.textContent=error.message;formFeedback.dataset.state='error'}finally{save.disabled=false}});load();
  })();`;
}
